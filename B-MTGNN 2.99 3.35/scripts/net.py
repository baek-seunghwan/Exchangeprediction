...
class gtnet(nn.Module):
    ...
    def forward(self, input, idx=None):
        ...
        # [1순위 안정화] assert 제거 + receptive field 기중 pad/crop
        # seq_len이 receptive_field보다 짧으면 pad, 길면 crop
        if seq_len < self.receptive_field:
            input = F.pad(input, (self.receptive_field - seq_len, 0, 0, 0))
        elif seq_len > self.receptive_field:
        ...
        for i in range(self.layers):
            ...
            if self.gcn_true:
                ...
                # [Patch 3] 대칭 A일 때 과증폭 완화: 평균 내기 (gconv1(A) + gconv2(A^T))
                # A가 대칭이면 A^T == A이므로, 동일한 정보를 두 번 더하지 않도록 0.5를 곱함
                x = 0.5 * (self.gconv1[i](x, adp) + self.gconv2[i](x, adp.transpose(1,0)))
            else:
