...
def compute_rse_rae(pred_2d, real_2d, eps=1e-12):
    """
    Compute RSE (Relative Squared Error) and RAE (Relative Absolute Error).
    
    pred_2d, real_2d: shape [T, N] (numpy or torch)
    
    RSE = sqrt(MSE / Var(real))
    RAE = MAE / MAD(real around mean)
    
    Returns: (rse_float, rae_float)
    """
    ...
    mse = float(np.mean((pred_f - real_f) ** 2))
    var = float(np.mean((real_f - real_f.mean()) ** 2))
    rse = (mse / max(var, eps)) ** 0.5
    
    mae = float(np.mean(np.abs(pred_f - real_f)))
    mad = float(np.mean(np.abs(real_f - real_f.mean())))
    rae = mae / max(mad, eps)
...
def save_integrated_metrics_txt(save_dir, split_name, scope_tag, rse, rae, node_names=None, fx_indices=None):
    ...
    with open(fpath, "w", encoding="utf-8") as f:
        f.write(f"{scope_tag}_{split_name}: rse={rse:.2f}, rae={rae:.2f}\n")
        if nodes_line:
            f.write(nodes_line + "\n")


# [개선 4] 로그수익률 기반 상관 그래프 생성 함수
def build_corr_adj_from_logret(raw_TN: torch.Tensor, topk: int = 20):
    """
    환율/시계열의 로그수익률(log-returns)로 상관 그래프 구성
    
    raw_TN: [T, N] (원 스케일, 양수 가정)
    topk: 각 노드당 연결할 상위 k개 이웃
    
    Returns: [N, N] adjacency matrix (top-k sparsified)
    """
    # 로그수익률 계산: log(p_t / p_{t-1})
    x = torch.log(raw_TN[1:] + 1e-8) - torch.log(raw_TN[:-1] + 1e-8)  # [T-1, N]
    ...
    num = x.t() @ x  # [N, N]
    den = torch.sqrt((x**2).sum(dim=0, keepdim=True)).t() @ torch.sqrt((x**2).sum(dim=0, keepdim=True))  # [N, N]
    
    corr = (num / (den + 1e-8)).abs()  # 절대값 상관계수 [N, N]
    corr.fill_diagonal_(0)  # 자기상관 제거
    
    # top-k sparsification
    k = min(topk, corr.size(1) - 1)
...
def plot_predicted_actual(predicted, actual, title, type, variance=None, confidence_95=None, scale=None, col_name=None):
    ...
    # x축 틱도 L 길이에 맞춰 안전하게
    p2 = [pp for pp in p if pp <= L]
    M2_2 = [M2[i] for i, pp in enumerate(p) if pp <= L]
    plt.xticks(ticks=p2, labels=M2_2, rotation='vertical', fontsize=13)
    plt.yticks(fontsize=13)
...
def evaluate_sliding_window(data, test_window, model, evaluateL2, evaluateL1, n_input, is_plot, delta_scale=1.0):
    ...
    for i in range(n_input, test_window.shape[0], data.out_len):
        ...
        with torch.no_grad():
            ...
            # [개선 5] delta_scale 적용 (Δ 축소로 Random Walk로 수렴)
            y_pred = delta_scale * out_delta + last.unsqueeze(0).repeat(out_delta.size(0), 1)  # [out_len, m] (레벨)
    ...
    scale_2d = data.scale.expand(test.size(0), data.m) #scale will have the max of each column (142 max values)
    bias_2d = getattr(data, 'bias', torch.zeros_like(data.scale)).expand(test.size(0), data.m)

    #inverse normalisation (z-score 복원: x = z*scale + bias)
    predict = predict * scale_2d + bias_2d
    test = test * scale_2d + bias_2d
    
    # 95% 예측구간: Testing에서는 PI(prediction interval) 기반 권장
    ci_series = None
    if is_plot and (std_series is not None):
        # predict/test에 scale을 곱했으니 std도 동일하게 scale
        # (z-score에서 std는 scale로만 곱함, bias 영향 없음)
        std_series = std_series * scale_2d

        # Testing: 예측분포(PI) 기반 95% 구간 -> 밴드가 보임
        ci_95_series = Z_95 * std_series
        
        # (개선) mean CI로 동동 싶게 보나은 경우:
        # ci_95_series = Z_95 * (std_series / math.sqrt(MC_SAMPLES))
        
        print("CI min/max (PI-based):", ci_95_series.min().item(), ci_95_series.max().item())
        print("CI mean:", ci_95_series.mean().item())
        ci_series = ci_95_series
    ...
    # [Patch 2] FX-only metric으로 early stopping 통일 (evaluate_sliding_window)
    fx_indices = data.fx_idx if (hasattr(data, 'fx_idx') and len(data.fx_idx) > 0) else list(range(data.m))
    
    # FX 컬럼만 슬라이스 [T, N_fx]
    predict_fx = predict[:, fx_indices] if predict.shape[1] > 0 else predict
    test_fx = test[:, fx_indices] if test.shape[1] > 0 else test
    
    #Root Relative Squared Error RRSE according to Lai et.al - numerator (FX-only)
    root_sum_squared = math.sqrt(torch.sum(torch.pow(test_fx - predict_fx, 2)).item()) #numerator
    
    #Root Relative Squared Error RRSE according to Lai et.al - denominator (FX-only)
    test_s = test_fx
    mean_all = torch.mean(test_s, dim=0) # calculate the mean of each FX column in test
    diff_r = test_s - mean_all.expand(test_s.size(0), test_s.size(1)) # subtract the mean from each element
    sum_squared_r = torch.sum(torch.pow(diff_r, 2))# square the result and sum over all elements
    root_sum_squared_r = math.sqrt(sum_squared_r.item())#denominator

    #RRSE according to Lai et.al (FX-only, early stopping 기준)
    rrse = root_sum_squared / max(root_sum_squared_r, 1e-8)
    print('rrse (FX-only)=', root_sum_squared, '/', root_sum_squared_r)

    #Relative Absolute Error RAE - denominator (FX-only)
    sum_absolute_r = torch.sum(torch.abs(diff_r))# absolute the result and sum over all elements - denominator
    #Relative Absolute Error RAE (FX-only)
    rae = torch.sum(torch.abs(test_fx - predict_fx)) / max(sum_absolute_r, 1e-8)
    rae = rae.item()
###########################################################################################################
    ...
    # ==========================================
    # At this point:
    # - predict: [T, N] torch tensor (predicted levels, not delta)
    # - test: [T, N] torch tensor (actual levels)
    # - data.m: number of nodes/columns (N)
    # - data.col: list of column names
    
    pred_2d = predict.clone()  # [T, N]
    real_2d = test.clone()     # [T, N]
    
    pred_2d_np = _to_numpy(pred_2d)
    real_2d_np = _to_numpy(real_2d)
    
    # Validate shapes
    if pred_2d_np.ndim != 2 or real_2d_np.ndim != 2:
        raise ValueError(f"Expected 2D arrays [T,N]. Got pred={pred_2d_np.shape}, real={real_2d_np.shape}")
    if pred_2d_np.shape != real_2d_np.shape:
        raise ValueError(f"Shape mismatch: pred={pred_2d_np.shape} vs real={real_2d_np.shape}")
    
    # Infer split_name from context (typically 'Testing' for this function)
    split_name = "Testing"
    
    # Infer node_names from data.col
    node_names = None
    if hasattr(data, 'col') and data.col is not None and len(data.col) == int(real_2d_np.shape[1]):
        node_names = list(data.col)
    node_names = _get_node_names_fallback(real_2d_np, node_names)
    
    # Get FX indices
    fx_keyword = getattr(args, 'fx_keyword', 'fx')
    fx_idx = _get_fx_indices(node_names, fx_keyword=fx_keyword)
    
    # Compute integrated metrics for ALL nodes
    rse_all, rae_all = compute_rse_rae(pred_2d_np, real_2d_np)
    
    # Compute integrated metrics for FX nodes only
    rse_fx, rae_fx = None, None
    if len(fx_idx) > 0:
        pred_fx = pred_2d_np[:, fx_idx]
        real_fx = real_2d_np[:, fx_idx]
        rse_fx, rae_fx = compute_rse_rae(pred_fx, real_fx)
    
    # Print integrated metrics based on scope
    integrated_scope = getattr(args, 'integrated_scope', 'both')
    if integrated_scope in ('all', 'both'):
        print(f"\n[Integrated ALL] {split_name}: rse={rse_all:.6f}, rae={rae_all:.6f} (N={real_2d_np.shape[1]})")
    if integrated_scope in ('fx', 'both'):
        if rse_fx is not None:
            print(f"[Integrated FX]  {split_name}: rse={rse_fx:.6f}, rae={rae_fx:.6f} (FX_N={len(fx_idx)})")
        else:
            print(f"[Integrated FX]  {split_name}: skipped (no fx nodes found by keyword='{fx_keyword}')")
    
    # Save integrated metrics to .txt files (if enabled)
    if getattr(args, 'save_integrated_txt', False):
        # Note: For evaluate_sliding_window, we don't have a separate save_dir parameter.
        # You can add one if needed. For now, we'll derive it from 'Testing' subdirectory assumption.
        # The save_dir path should be constructed similarly to where save_metrics_1d saves files.
        # A typical pattern is: os.path.join(project_root, 'model', 'Bayesian', 'Testing')
        # For flexibility, we'll check if Testing folder exists relative to script location.
        script_dir = os.path.dirname(os.path.abspath(__file__))
        test_save_dir = os.path.join(script_dir, '..', 'model', 'Bayesian', 'Testing')
        test_save_dir = os.path.normpath(test_save_dir)
        
        if integrated_scope in ('all', 'both'):
            save_integrated_metrics_txt(test_save_dir, split_name, 'ALL', rse_all, rae_all, 
        ...
        if integrated_scope in ('fx', 'both') and rse_fx is not None:
            save_integrated_metrics_txt(test_save_dir, split_name, 'FX', rse_fx, rae_fx,
                                       node_names=node_names, fx_indices=fx_idx)
...
def evaluate(data, X, Y, model, evaluateL2, evaluateL1, batch_size, is_plot, delta_scale=1.0):
    # RSE 최적화: 빈 배치 방어
    ...
    for X, Y in data.get_batches(X, Y, batch_size, False):
        ...
        # [개선 5] delta_scale 적용 (Δ 축소로 Random Walk로 수렴)
        output = delta_scale * out_delta + last_rep  # [B, out_len, m] (레벨로 복원)
        ...
        bias = getattr(data, 'bias', torch.zeros_like(data.scale)).expand(Y.size(0), Y.size(1), data.m)
        
        #inverse normalisation (z-score 복원: x = z*scale + bias)
        output = output * scale + bias
        Y = Y * scale + bias
        var = var * scale * scale  # 분산은 scale의 제곱으로 곱하기
    ...
    # [Patch 2] FX-only metric으로 early stopping 통일
    # predict/test shape: [B, out_len, m]
    # FX 인덱스 확인
    fx_indices = data.fx_idx if (hasattr(data, 'fx_idx') and len(data.fx_idx) > 0) else list(range(data.m))
    ...
    #RRSE according to Lai et.al - numerator (FX-only)
    root_sum_squared= math.sqrt(torch.sum(torch.pow(test_fx - predict_fx, 2)).item()) #numerator
    
    #RRSE according to Lai et.al - denominator (FX-only)
    test_s = test_fx
    mean_all = torch.mean(test_s, dim=(0,1)) # calculate the mean of each FX column in test
    diff_r = test_s - mean_all.expand(test_s.size(0), test_s.size(1), test_s.size(2)) # subtract the mean from each element
    sum_squared_r = torch.sum(torch.pow(diff_r, 2))# square the result and sum over all elements
    root_sum_squared_r = math.sqrt(sum_squared_r.item())#denominator

    #RRSE according to Lai et.al (FX-only)
    rrse = root_sum_squared / max(root_sum_squared_r, 1e-8) #RRSE (FX-only, early stopping 기준)

    #Relative Absolute Error RAE (FX-only)
    sum_absolute_r = torch.sum(torch.abs(diff_r))# absolute the result and sum over all elements - denominator
    rae = torch.sum(torch.abs(test_fx - predict_fx)) / max(sum_absolute_r, 1e-8) # RAE (FX-only)
    rae = rae.item()
...
def train(data, X, Y, model, criterion, optim, batch_size):
    ...
    for X, Y in data.get_batches(X, Y, batch_size, True):
        ...
        # [개선 1] SmoothL1Loss (아웃라이어에 덜 민감) - reduction='none'으로 가중치 적용 가능
        loss_raw = F.smooth_l1_loss(out_delta, ty_delta, reduction='none')  # [B, out_len, m]
...
parser.add_argument('--L1Loss', action=argparse.BooleanOptionalAction, default=True, help='use L1 loss')
parser.add_argument('--normalize', type=int, default=2)
parser.add_argument('--device',type=str,default='cuda:0',help='device (cuda:0 or cpu)')
parser.add_argument('--gcn_true', action=argparse.BooleanOptionalAction, default=True, help='whether to add graph convolution layer - use --no-gcn_true to disable')
...
parser.add_argument('--dropout',type=float,default=0.05,help='dropout rate (권장: 0.15->0.05, 36개월)')
parser.add_argument('--subgraph_size',type=int,default=20,help='k (권장: 10->20, 36개월)')
...
parser.add_argument('--seq_in_len',type=int,default=19,help='input sequence length (receptive_field과 일치 필수)')
parser.add_argument('--seq_out_len',type=int,default=12,help='output sequence length (권장: 12, 3회 roll=36개월)')
parser.add_argument('--horizon', type=int, default=1, help='lead time (권장: 1 = next-step)')
parser.add_argument('--layers',type=int,default=3,help='number of layers')

parser.add_argument('--batch_size',type=int,default=32,help='batch size')
parser.add_argument('--lr',type=float,default=0.0005,help='learning rate (권장: 0.001->0.0005, 36개월 직접예측)')
...
# [개선 5] 예측 Δ 축소 계수 (Shrinkage: Random Walk 쪽으로 당기기)
parser.add_argument('--delta_scale', type=float, default=1.0,
                    help='shrink factor for predicted delta when reconstructing level (0~1, recommend: 0.2~0.7 for FX)')
...
parser.add_argument('--epochs',type=int,default=50,help='epochs (권장: 50->200, 36개월 학습 안정화)')
...
def main(experiment):
    ...
    for q in range(1):
        ...
        # [디버그] bias/scale 존재 확인
        assert hasattr(Data, "bias") and hasattr(Data, "scale"), "Data must have 'bias' and 'scale' attributes for z-score normalization"
        print(f"[INFO] Normalization check: bias shape={Data.bias.shape}, scale shape={Data.scale.shape}")
        print(f"[INFO] Train data mean={Data.dat[:int(0.6*Data.n)].mean(dim=0)[:3]}, std={Data.dat[:int(0.6*Data.n)].std(dim=0)[:3]} (first 3 cols)")
        ...
        # [개선 2] 그래프: 로그수익률(log-return) 기반으로 강제 적용
        train_end = int(0.6 * Data.n)
        print("\n[GRAPH] Building log-return correlation graph from training data (FX-specific)")
        raw_train = torch.clamp(Data.rawdat[:train_end, :].clone(), min=1e-6)  # 0/음수 방어
        A_corr = build_corr_adj_from_logret(raw_train, topk=args.subgraph_size)
        Data.adj = A_corr.to(device)
        print(f"[GRAPH] log-return adjacency created: shape={Data.adj.shape}, edges={Data.adj.nonzero().size(0)}")
        ...
        optim = Optim(
            ...
            lr_gamma=args.lr_gamma  # [4순위 추가] lr_gamma를 optim에 전달
        )
        ...
        try:
            ...
            for epoch in range(1, args.epochs + 1):
                ...
                val_loss, val_rae, val_corr, val_smape = evaluate(Data, Data.valid[0], Data.valid[1], model, evaluateL2, evaluateL1,
                                                 args.batch_size, False, args.delta_scale)
                print(
                ...
                optim.lr_step()
                print(f"  → current lr: {optim.get_lr():.6f}", flush=True)
                ...
                if (not math.isnan(val_corr)) and val_loss < best_rse:
                    ...
                    test_acc, test_rae, test_corr, test_smape = evaluate_sliding_window(Data, Data.test_window, model, evaluateL2, evaluateL1,
                                           args.seq_in_len, False, args.delta_scale) 
                    print('********************************************************************************************************')
    ...
    vtest_acc, vtest_rae, vtest_corr, vtest_smape = evaluate(Data, Data.valid[0], Data.valid[1], model, evaluateL2, evaluateL1,
                                         args.batch_size, True, args.delta_scale)

    test_acc, test_rae, test_corr, test_smape = evaluate_sliding_window(Data, Data.test_window, model, evaluateL2, evaluateL1,
                                         args.seq_in_len, True, args.delta_scale) 
    print('********************************************************************************************************')    
