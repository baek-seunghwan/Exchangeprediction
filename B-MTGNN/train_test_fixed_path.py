...
def evaluate_sliding_window(data, test_window, model, evaluateL2, evaluateL1, n_input, is_plot):
    ...
    if is_plot:
        ...
        # # 평균 RSE/RAE 저장 (FX 또는 ALL 기준)
        from pathlib import Path
        PROJECT_DIR = Path(__file__).resolve().parents[1]
        save_path = str(PROJECT_DIR / 'model' / 'Bayesian' / 'Testing')
        Path(save_path).mkdir(parents=True, exist_ok=True)
        
        avg_file = Path(save_path) / 'Average_of_each_call.txt'
        
        avg_file = os.path.join(save_path, 'Average_of_each_call.txt')
        with open(avg_file, 'w', encoding='utf-8') as f:
            if rse_fx is not None:
                f.write(f'rse:{rse_fx:.3f}\n')
                f.write(f'rae:{rae_fx:.3f}\n\n')
            else:
                f.write(f'rse:{rse_all:.3f}\n')
                f.write(f'rae:{rae_all:.3f}\n\n')
...
# 수정된 코드
parser.add_argument('--data', type=str, default='/content/drive/MyDrive/Cyber-trend-forecasting-main/Cyber-trend-forecasting-main/B-MTGNN/data/ExchangeRate_dataset.csv',
                    help='location of the data file')
parser.add_argument('--save', type=str, default='/content/drive/MyDrive/Cyber-trend-forecasting-main/Cyber-trend-forecasting-main/B-MTGNN/model/Bayesian/model.pt', help='path to save the final model')
...
parser.add_argument('--dropout',type=float,default=0.3,help='dropout rate (권장: 0.15->0.05, 36개월)')
...
parser.add_argument('--epochs',type=int,default=300,help='epochs (권장: 50->200, 36개월 학습 안정화)')
...
def main(experiment):
    ...
    print('********************************************************************************************************')
    return vtest_acc, vtest_rae, vtest_corr, vtest_smape, test_acc, test_rae, test_corr, test_smape, test_pred, test_true
...
if __name__ == "__main__":
    ...
    # 모든 실행의 예측값을 저장할 리스트
    all_preds = []
    final_true = None # 실제값은 매번 똑같으므로 한 번만 저장
    ...
    for i in range(ensemble_runs):
        print(f"\n[Ensemble Run {i+1}/{ensemble_runs}]")
        
        # main 함수에서 예측값 배열까지 받아옴
        val_acc, val_rae, val_corr, val_smape, test_acc, test_rae, test_corr, test_smape, test_pred, test_true = main(i)
    ...
    # 2. 앙상블(평균) 예측의 성능 계산
    # (Shape: [Runs, Time, Nodes]) -> [Time, Nodes] 로 평균
    ensemble_pred = np.mean(np.array(all_preds), axis=0)
    
    # 오차율 재계산 (기존 compute_rse_rae 함수 활용)
    # FX(환율) 인덱스 찾기
    fx_keyword = getattr(args, 'fx_keyword', 'fx')
    # node names가 없으면 임시 생성
    node_names = [f"node_{i}" for i in range(final_true.shape[1])]
    
    # Data 객체에 접근할 수 없으므로, fx인덱스를 직접 찾거나 전체 RSE를 계산
    # 여기서는 전체(ALL)와 FX를 분리해서 계산 시도
    ...
    # 만약 'fx' 컬럼만 따로 보고 싶다면 (Col name을 알기 어려우므로 util함수 재사용이 까다로움)
    # 하지만 main함수 안에서 이미 출력했으므로, 여기서는 '얼마나 줄었는지' 비교에 집중합니다.
    
    improvement_rse = np.mean(acc) - ens_rse_all
    print(f"\n>> Improvement by Ensemble: {improvement_rse:.6f} (RSE reduction)")
    
    # 앙상블 결과 저장 (옵션)
    save_path = '/content/drive/MyDrive/model/Bayesian/Testing/Ensemble_Result.txt'
    with open(save_path, "w", encoding="utf-8") as f:
        f.write(f"Ensemble Runs: {ensemble_runs}\n")
        f.write(f"ALL RSE: {ens_rse_all:.6f}\n")
        f.write(f"ALL RAE: {ens_rae_all:.6f}\n")
    print(f"Ensemble metrics saved to {save_path}")