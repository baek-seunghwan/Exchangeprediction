...
def save_metrics_1d(predict, test, title, type):
    ...
    if sum_absolute_r == 0:
        rae = torch.tensor(0.0)
    else:
        rae = sum_absolute_diff / sum_absolute_r
    rae = rae.item()

    # ----- 2) title 정리(BOM, 슬래시, 공백 처리) -----
    raw_title = str(title) if title is not None else ""
    safe_title = raw_title.replace('\ufeff', '').strip()  # BOM 제거 + 양끝 공백 제거
    if safe_title == "":
        safe_title = "node"
    safe_title = safe_title.replace('/', '_')  # 경로 깨지는 문자 방지

    # ----- 3) 디렉터리 생성 -----
    out_dir = os.path.join("model", "Bayesian", type)
    os.makedirs(out_dir, exist_ok=True)

    # ----- 4) 파일에 저장 -----
    file_path = os.path.join(out_dir, f"{safe_title}_{type}.txt")
...
def plot_predicted_actual(predict, Ytest, title, type, variance, confidence_95):
    """
    predict, Ytest: 1D numpy array or tensor (sliding window 결과)
    title: 노드 이름 (컬럼명) – BOM, 공백 등 정리 필요
    type: 'Training' / 'Validation' / 'Testing'
    """

    # ----- 1) title 정리 (BOM, 공백 제거) -----
    raw_title = str(title) if title is not None else ""
    safe_title = raw_title.replace('\ufeff', '').strip()
    if safe_title == "":
        # 이름이 완전히 비어 있으면 대체 이름 사용
        safe_title = "node"
    ...
    # ----- 3) 실제 값/예측 값/신뢰구간을 이용해 그래프 그리기 -----
    # 필요에 따라 원래 코드의 plot 부분을 여기 안에 넣으면 됩니다.
    plt.figure(figsize=(8, 4))

    # numpy/tensor 섞여 있어도 되도록 처리
    import numpy as np
    ...
    if hasattr(Ytest, "detach"):
        y_np = Ytest.detach().cpu().numpy()
    else:
        y_np = np.array(Ytest)

    x_axis = np.arange(len(predict_np))

    plt.plot(x_axis, y_np, label="Actual")
    plt.plot(x_axis, predict_np, label="Predicted")

    # 분산·신뢰구간이 있으면 함께 그림
    if variance is not None and confidence_95 is not None:
        if hasattr(confidence_95, "detach"):
            conf_np = confidence_95.detach().cpu().numpy()
        else:
            conf_np = np.array(confidence_95)
        upper = predict_np + conf_np
        lower = predict_np - conf_np
        plt.fill_between(x_axis, lower, upper, alpha=0.2, label="95% CI")

    plt.xlabel("Time (sliding steps)")
    plt.ylabel("Normalized value")
    plt.title(f"{safe_title} - {type}")
    plt.legend()
    plt.tight_layout()

    # ----- 4) PNG로 저장 (PDF 대신) -----
    file_name = f"{safe_title}_{type}.png"
    ...
    for i in range(n_input, test_window.shape[0],data.out_len):
        ...
        for _ in range(num_runs):
            with torch.no_grad():
                ...
                # Truncate y_pred to match y_true if necessary
...
###########################################################################################################
    ...
    index = (sigma_g != 0)
    correlation = np.divide(((predict - mean_p) * (Ytest - mean_g)).mean(axis=0), (sigma_p * sigma_g), out=np.zeros_like(sigma_p), where=(sigma_p * sigma_g) != 0)  # Pearson's correlation coefficient?
    correlation = correlation[index].mean() if index.any() else 0.0
...
def evaluate(data, X, Y, model, evaluateL2, evaluateL1, batch_size, is_plot):
    ...
    for X, Y in data.get_batches(X, Y, batch_size, False):
        ...
        scale_Y = data.scale.expand(Y.size(0), Y.size(1), data.m)
        scale_output = data.scale.expand(output.size(0), output.size(1), data.m)

        #inverse normalisation
        output*=scale_output
        Y*=scale_Y
        var*=scale_output
        confidence*=scale_output
...
def train(data, X, Y, model, criterion, optim, batch_size):
    ...
    for X, Y in data.get_batches(X, Y, batch_size, True):
        ...
        for j in range(args.num_split):
            ...
            scale = data.scale.expand(output.size(0), Y.size(1), data.m)
            scale = scale[:,:,:] 
...
parser.add_argument('--data', type=str, default='../FX_Data/ExchangeRate_dataset.csv',
                    help='location of the FX data file')    # Change default data dirirectory
parser.add_argument('--log_interval', type=int, default=2000, metavar='N',
                    help='report interval')
parser.add_argument('--save', type=str, default='model/Bayesian/model_new.pt',
                    help='path to save the final model')
parser.add_argument('--optim', type=str, default='adam')
parser.add_argument('--L1Loss', type=bool, default=True)
parser.add_argument('--normalize', type=int, default=2)
parser.add_argument('--device', type=str, default='cpu', help='', choices=['cuda:0', 'cuda:1', 'cpu'])
parser.add_argument('--gcn_true', type=bool, default=True, help='whether to add graph convolution layer')
parser.add_argument('--buildA_true', type=bool, default=True, help='whether to construct adaptive adjacency matrix')
parser.add_argument('--gcn_depth',type=int,default=2,help='graph convolution depth')
parser.add_argument('--num_nodes',type=int,default=32,help='number of nodes/variables')    # origin: default=142
...
parser.add_argument('--seq_out_len',type=int,default=8,help='output sequence length')   # origin: 36
...
def main(experiment):
    ...
    for q in range(10):
        ...
        model = gtnet(args.gcn_true, args.buildA_true, gcn_depth, args.num_nodes,
                    ...
                    layers=layer, propalpha=prop_alpha, tanhalpha=tanh_alpha, layer_norm_affline=False) # subgraph_size=k
