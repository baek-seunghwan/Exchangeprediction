...
class DataLoaderS(object):
    ...
    def _batchify(self, idx_set, horizon):
        ...
        if n <= self.out_len:
            raise ValueError(
                f"Not enough samples after windowing: n={n}, out_len={self.out_len}. "
                "Decrease out_len or increase data length."
            )
    ...
    def get_batches(self, inputs, targets, batch_size, shuffle=True):
        # batch_size 방어: 실수로 argparse.Namespace 등을 넘겼을 때를 대비
        if not isinstance(batch_size, (int, np.integer)):
            if hasattr(batch_size, 'batch_size'):
                batch_size = int(getattr(batch_size, 'batch_size'))
            else:
