...
def train(data, X, Y, model, criterion, optim, batch_size):
    # Defensive: batch_size must be int
    if not isinstance(batch_size, int):
        try:
            # common bug: batch_size receives argparse.Namespace
            batch_size = int(getattr(batch_size, 'batch_size', batch_size))
        except Exception:
            batch_size = 8
...
parser.add_argument('--dropout',type=float,default=0.1,help='dropout rate (권장: 0.15->0.05, 36개월)')
...
parser.add_argument('--loss', type=str, default='huber', choices=['mse','huber'],
                    help='Training loss on deltas (mse or huber).')
parser.add_argument('--huber_beta', type=float, default=1.0,
                    help='Huber transition point for smooth_l1_loss.')
parser.add_argument('--horizon_decay', type=float, default=0.0,
                    help='If >0, apply exp(-decay*t) weights across forecast horizon (t=0..H-1).')
parser.add_argument('--lr_scheduler', type=str, default='plateau', choices=['none','plateau'],
                    help='Learning-rate scheduler. plateau uses ReduceLROnPlateau on valid RSE.')
...
def main(experiment):
    ...
    for q in range(1):
        ...
        # Scheduler selection:
        # - If args.lr_scheduler == 'plateau', use ReduceLROnPlateau here and disable Optim's internal ExponentialLR.
        # - Otherwise, fall back to Optim.lr_step() (ExponentialLR when lr_gamma is set).
        lr_gamma_to_use = None if getattr(args, "lr_scheduler", "") == "plateau" else getattr(args, "lr_gamma", None)
        ...
        optim = Optim(
            ...
            lr_gamma=lr_gamma_to_use  # [4순위 추가] lr_gamma를 optim에 전달
        )

        plateau_scheduler = None
        if getattr(args, "lr_scheduler", "") == "plateau":
        ...
        try:
            ...
            for epoch in range(1, args.epochs + 1):
                ...
                if plateau_scheduler is not None:
                    plateau_scheduler.step(float(val_loss))
                else:
                    optim.lr_step()
