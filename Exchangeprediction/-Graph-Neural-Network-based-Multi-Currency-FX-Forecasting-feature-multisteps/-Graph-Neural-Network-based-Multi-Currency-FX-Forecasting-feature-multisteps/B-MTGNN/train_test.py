<<<<<<< HEAD:Cyber-trend-forecasting-main 3/B-MTGNN/train_test.py
import argparse
import os
import math
import time
...
if __name__ == "__main__":
    ...
    print("mean\t{:5.4f}\t{:5.4f}".format(np.mean(acc), np.mean(rae)))
    print("std\t{:5.4f}\t{:5.4f}".format(np.std(acc), np.std(rae)))
>>>>>>> parent of 8bb80fe (지원님껄로 바꿈):Exchangeprediction/-Graph-Neural-Network-based-Multi-Currency-FX-Forecasting-feature-multisteps/-Graph-Neural-Network-based-Multi-Currency-FX-Forecasting-feature-multisteps/B-MTGNN/train_test.py
