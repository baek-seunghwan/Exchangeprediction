...
def evaluate_sliding_window(data, test_window, model, evaluateL2, evaluateL1, n_input, is_plot, split_type='Testing', bias_offset=None, return_arrays=False):
    ...
    # ============================================
    # 1단계. 테스트 구간 전체를 '한 번에' 예측하는 Direct Rollout 모드 추가
    # ============================================
    if split_type == 'Testing' and args.rollout_mode == 'direct':
        print('Using Direct Multi-Step Rollout (No recursive accumulation)')
        
        # 테스트 시작 시점(딱 n_input 시점까지)의 100% 진짜 과거 데이터 덩어리 1개만 추출
        x_input = test_window[0:n_input, :].clone()
        ...
        # 단 1번만 RevIN 파라미터 계산 (절대 미래가 섞이지 않은 진짜 잣대)
        w_mean = X.mean(dim=-1, keepdim=True)
        w_std = X.std(dim=-1, keepdim=True)
        w_std[w_std == 0] = 1
        X = (X - w_mean) / w_std
        ...
        # 단 한 번의 추론으로 미래 12개월(args.seq_out_len) 전체 궤적 그리기
        num_runs = 10
        outputs = []
        for _ in range(num_runs):
            with torch.no_grad():
                output = model(X)
                # 모델이 훈련받은 대로 12개월(seq_out_len) 전체를 그대로 반환
                y_pred_all = output[-1, :args.seq_out_len, :, -1].clone()
                # Z-Score 정규화였던 값을 1번만 구해둔 진짜 잣대로 완벽히 복원
                y_pred_all = y_pred_all * ws + wm
                outputs.append(y_pred_all)
        
        outputs = torch.stack(outputs)
        predict = torch.mean(outputs, dim=0) # [seq_out_len, N]
        
        # [NEW] Direct Rollout에도 닻(Anchor) 로직 적용
        # Direct는 12개월(seq_out_len)을 한 번에 뽑으므로, 미래로 갈수록 닻의 영향력을 줄이는(Decay) 것이 정석입니다.
        alpha_base = min(max(args.anchor_focus_to_last, 0.0), 1.0)
        if alpha_base > 0 and args.focus_targets == 1:
            focus_cols = get_focus_columns(data)
            if focus_cols:
                focus_idx = torch.tensor(focus_cols, device=data.device)
                y_focus = torch.index_select(predict, dim=1, index=focus_idx) # [seq_out_len, focus]
                
                # 테스트 윈도우 시작 직전(n_input-1)의 '실제 관측값'을 닻으로 사용
                last_obs = test_window[n_input - 1, :].clone()
                last_focus = torch.index_select(last_obs, dim=0, index=focus_idx).unsqueeze(0) # [1, focus]
                
                # 시간에 따른 감쇠(Decay) 비율 생성. 가까운 미래는 닻 강하게, 먼 미래는 약하게.
                # 예: 1개월 차 = alpha_base, 12개월 차 = alpha_base * 0.1
                steps = y_focus.shape[0]
                decay_factors = torch.linspace(1.0, 0.1, steps, device=data.device).unsqueeze(1) # [seq_out_len, 1]
                alpha_t = alpha_base * decay_factors # [seq_out_len, 1]
                
                # 예측값과 실제 관측값을 시간 가변 비율로 융합
                y_focus = (1.0 - alpha_t) * y_focus + alpha_t * last_focus
                
                # 융합된 값을 기존 예측 텐서에 덮어쓰기
                predict[:, focus_idx] = y_focus
                
        variance = torch.var(outputs, dim=0)
        std_dev = torch.std(outputs, dim=0)
        confidence_95 = 1.96 * std_dev / torch.sqrt(torch.tensor(num_runs))

        # 정답(Actual)도 테스트 기간(12개월) 맞춰서 1번에 뜯어냄
        test = test_window[n_input:n_input+args.seq_out_len, :].clone()
        
        # 모델의 출력 길이에 맞게 테스트 데이터 자르기 방지
        actual_len = min(predict.shape[0], test.shape[0])
        predict = predict[:actual_len, :]
        test = test[:actual_len, :]
        variance = variance[:actual_len, :]
        confidence_95 = confidence_95[:actual_len, :]
        
    # ============================================
    # 2단계. Hybrid Rollout 모드 (Recursive + Direct 가중 평균 앙상블)
    # ============================================
    elif split_type == 'Testing' and args.rollout_mode == 'hybrid':
        print('Using Hybrid Multi-Step Rollout (Recursive + Direct Fusion)')
        
        # [A] Direct 예측 전체 미리 뽑아두기 (12개월치)
        x_input_direct = test_window[0:n_input, :].clone()
        X_dir = torch.unsqueeze(x_input_direct, dim=0).unsqueeze(1).transpose(2, 3).to(torch.float)
        
        w_mean_dir = X_dir.mean(dim=-1, keepdim=True)
        w_std_dir = X_dir.std(dim=-1, keepdim=True)
        w_std_dir[w_std_dir == 0] = 1
        X_dir = (X_dir - w_mean_dir) / w_std_dir
        
        wm_dir = w_mean_dir[0, 0, :, 0]
        ws_dir = w_std_dir[0, 0, :, 0]
        ...
        for _ in range(10):
            with torch.no_grad():
                output = model(X_dir)
                y_pred_all = output[-1, :args.seq_out_len, :, -1].clone()
                y_pred_all = y_pred_all * ws_dir + wm_dir
                outputs_dir.append(y_pred_all)
        
        predict_direct_all = torch.mean(torch.stack(outputs_dir), dim=0) # [seq_out_len, N]
        
        # [B] Recursive 루프 돌면서 매 스텝 융합(Fusion) 연산
        x_input = test_window[0:n_input, :].clone()
        
        for i in range(n_input, test_window.shape[0]):
            step_idx = i - n_input # 0, 1, 2...
            
            X = torch.unsqueeze(x_input, dim=0).unsqueeze(1).transpose(2, 3).to(torch.float)
            w_mean = X.mean(dim=-1, keepdim=True)
            w_std = X.std(dim=-1, keepdim=True)
            w_std[w_std == 0] = 1
            X = (X - w_mean) / w_std
            ...
            for _ in range(num_runs):
                with torch.no_grad():
                    output = model(X)
                    y_pred_single = output[-1, 0, :, -1].clone()
                    y_pred_single = y_pred_single * ws + wm
                    outputs_rec.append(y_pred_single.unsqueeze(0))
            
            y_pred_rec = torch.mean(torch.stack(outputs_rec), dim=0) # [1, N]
            var = torch.var(torch.stack(outputs_rec), dim=0)
            std_dev = torch.std(torch.stack(outputs_rec), dim=0)
            confidence = 1.96 * std_dev / torch.sqrt(torch.tensor(num_runs))
            
            # --- 가중 평균 Fusion 로직 ---
            if step_idx < args.seq_out_len:
                # 시간이 흐를수록 Direct(장기 예측력 증폭) 비중 증가
                w_dir = step_idx / max(1, (args.seq_out_len - 1)) 
                w_rec = 1.0 - w_dir
                
                y_pred_dir_step = predict_direct_all[step_idx].unsqueeze(0) # [1, N]
                y_pred_hybrid = (w_rec * y_pred_rec) + (w_dir * y_pred_dir_step)
            else:
                y_pred_hybrid = y_pred_rec
            
            # 다음 윈도우 입력을 위해 앙상블된 결과를 넘겨줍니다.
            next_chunk = y_pred_hybrid
            x_input = torch.cat([x_input[1:, :].clone(), next_chunk.detach().clone()], dim=0)
            
            if predict is None:
                predict = y_pred_hybrid
                test = y_true
                variance = var
                confidence_95 = confidence
            else:
                predict = torch.cat((predict, y_pred_hybrid))
                test = torch.cat((test, y_true))
                variance = torch.cat((variance, var))
                confidence_95 = torch.cat((confidence_95, confidence))
                
    # ============================================
    # 3단계. 기존 Recursive 로직 (rollout_mode == 'recursive' 또는 'teacher_forced')
    # ============================================
    else:
        for i in range(n_input, test_window.shape[0]):
            X = torch.unsqueeze(x_input, dim=0)
            ...
            for _ in range(num_runs):
                with torch.no_grad():
                    output = model(X)
                    y_pred_single = output[-1, 0, :, -1].clone() # Always take the 1st step explicitly to avoid issues
                    y_pred_single = y_pred_single * ws + wm
                    outputs.append(y_pred_single.unsqueeze(0)) # Shape [1, N]
    ...
    # Z-Score 대응: mean이 있으면 더해줍니다.
    mean = getattr(data, 'mean', torch.zeros(data.m).to(data.device)).expand(test.size(0), data.m)

    predict = predict * scale + mean
    test = test * scale + mean
    variance *= scale # Variance and confidence are not shifted
    confidence_95 *= scale

    # =========================================================
    # [수정] Direct 모드에서 Anchor를 사용했을 때 Debias와의 충돌 방지
    # Anchor(닻)는 이미 예측선을 정답지에 가깝게 고의적으로 끌어내리는(수직 이동) 역할을 합니다.
    # 그런데 여기에 val_mean_error(검증셋 오차 편향)까지 또 빼버리면 '이중 차감(Double Debiasing)'이 발생하여 차트가 지하로 박힙니다.
    # 따라서 Anchor 값이 0보다 크면 Debias 로직을 무효화(Bypass)합니다.
    # =========================================================
    if bias_offset is not None:
        if args.rollout_mode == 'direct' and args.anchor_focus_to_last > 0:
            print(f"[Warning] Anchor ({args.anchor_focus_to_last}) is active. Bypassing val_mean_error debias to prevent double-shifting.")
        else:
    ...
    if is_plot:
        ...
        for v in range(data.m):
            ...
            # [수정] 사용자의 요청에 따라 US, KR, JP 통화 노드만 엄격히 하드코딩 필터링
            is_target = False
            lower_name = raw_name.lower()
            
            # 정확히 일치하거나 핵심 키워드가 있는 지 확인 (Us_Trade Weighted Dollar Index, kr_fx, jp_fx)
            if 'us_trade' in lower_name or 'kr_fx' in lower_name or 'jp_fx' in lower_name:
                is_target = True
        ...
        for j in range(args.num_split):
           ...
           # ===== [수정] Target-Weighted & Trend Loss 강화 =====
            ...
            if output.size(1) > 1:
                ...
                # [NEW: Plan A] Temporal Smoothness Loss (Total Variation)
                # 예측선의 절대적인 꺾임(지그재그) 자체를 처벌하여 곡선을 극도로 부드럽게 만듦
            ...
            if args.focus_targets == 1 and args.focus_only_loss == 1:
                ...
                if focus_cols:
                    ...
                    if grad_loss is not None:
                        ...
                        loss += 0.5 * grad_focus.mean() # Reduced from 2.0 to stabilize trends
                        
                    # [NEW: Plan A] Smoothness Penalty (평활화)
...
parser.add_argument('--save', type=str, default='model/Bayesian/model.pt', help='path to save the final model')
...
parser.add_argument('--focus_only_loss', type=int, default=1, choices=[0, 1], help='1 to compute training loss only on focus nodes')
parser.add_argument('--focus_weight', type=float, default=1.0, help='weight ratio (0~1) for focus nodes in validation metric (1.0 = focus nodes only)')
parser.add_argument('--focus_target_gain', type=float, default=5.0, help='focus target loss weight gain multiplier')
parser.add_argument('--anchor_focus_to_last', type=float, default=0.4, help='0~1 level anchoring strength for focus columns during evaluation/forecast')
...
parser.add_argument('--rse_report_mode', type=str, default='target', choices=['all', 'target'], help='whether to report RSE for all nodes or target nodes by default')
...
parser.add_argument('--bias_penalty', type=float, default=0.2, help='lambda for bias penalization in training loss (0 to disable)')
...
parser.add_argument('--smoothness_penalty', type=float, default=1.0, help='TV loss weight to encourage smooth curves in Direct forecasting')
parser.add_argument('--plot_focus_only', type=int, default=1, help='1 to plot/save only focus nodes')
...
if args.target_profile == 'triple_050':
    ...
    if '--lr' not in sys.argv: args.lr = 0.0003
    if '--seq_in_len' not in sys.argv: args.seq_in_len = 36
    if '--bias_penalty' not in sys.argv: args.bias_penalty = 0.2
    if '--smoothness_penalty' not in sys.argv: args.smoothness_penalty = 0.5
    if '--dropout' not in sys.argv: args.dropout = 0.1
    if '--anchor_focus_to_last' not in sys.argv: args.anchor_focus_to_last = 0.0
    if '--ss_prob' not in sys.argv: args.ss_prob = 0.0
    if '--seed' not in sys.argv: args.seed = 123
    if '--focus_weight' not in sys.argv: args.focus_weight = 1.0
    if '--focus_target_gain' not in sys.argv: args.focus_target_gain = 5.0
    if '--focus_only_loss' not in sys.argv: args.focus_only_loss = 1
    if '--rollout_mode' not in sys.argv: args.rollout_mode = 'direct'
    if '--debias_mode' not in sys.argv: args.debias_mode = 'val_mean_error'
    if '--debias_apply_to' not in sys.argv: args.debias_apply_to = 'focus'
    if '--bias_penalty_scope' not in sys.argv: args.bias_penalty_scope = 'focus'
    if '--use_graph' not in sys.argv: args.use_graph = 0
    if '--normalize' not in sys.argv: args.normalize = 3 
    print('[target_profile] applied: triple_050 (optimized for stable RSE)')
