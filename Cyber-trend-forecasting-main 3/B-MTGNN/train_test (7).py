    ...
    for i in range(n_input, test_window.shape[0], data.out_len):
        ...
        if getattr(args, 'enable_clamping', False):
            ...
            for tt in range(y_adj.shape[0]):
                delta = torch.abs(last) * pct
                delta = torch.clamp(delta, min=1e-6)
                lower = last - delta
                upper = last + delta
                y_adj[tt, :] = torch.max(torch.min(y_adj[tt, :], upper), lower)
                last = y_adj[tt, :].clone()
            y_pred = y_adj
    ...
    for X, Y in data.get_batches(X, Y, batch_size, False):
        ...
        with torch.no_grad():
            for _ in range(num_runs):
                ...
                # --- shape safety: normalize to (B, T, M) without collapsing batch dim ---
                if output.dim() == 4:
                    # common cases: (B,1,T,M) or (B,T,M,1)
                    if output.size(1) == 1:
                        output = output.squeeze(1)
                    elif output.size(-1) == 1:
                        output = output.squeeze(-1)

                if output.dim() == 2:          # (T,M) -> (1,T,M)
                    output = output.unsqueeze(0)
                elif output.dim() == 1:        # (M) -> (1,1,M)
                    output = output.view(1, 1, -1)

                if output.dim() != 3:
                    raise RuntimeError(f"[evaluate] unexpected model output shape: {tuple(output.shape)}")
        ...
        # Scale back to original units (broadcast-safe, no in-place ops)
        scale = data.scale
        if isinstance(scale, np.ndarray):
            scale = torch.from_numpy(scale)
        scale = scale.to(output.device).type_as(output)
        scale = scale.view(1, 1, data.m)
...
parser.add_argument('--enable_smoothing', action='store_true', help='apply EMA smoothing to rolling predictions')
parser.add_argument('--smooth_alpha', type=float, default=0.7, help='EMA alpha (higher=less smoothing)')
parser.add_argument('--enable_clamping', action='store_true', help='clamp rolling prediction changes vs last value')
