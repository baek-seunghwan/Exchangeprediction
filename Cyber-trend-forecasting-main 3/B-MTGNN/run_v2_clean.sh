#!/bin/bash
# ====================================================================
# Strategy v2: Remove upward-bias penalties + z-score + longer context
#
# Key changes from graph_diff:
#   - normalize=3 (z-score) instead of 2 (max-abs)
#   - seq_in_len=36 (3 years context, teammate setting)
#   - lag_penalty=0, lag_sign=0 (REMOVE upward trend enforcement)
#   - grad_loss_weight=0 (REMOVE training gradient enforcement)
#   - smoothness_penalty=0 (no TV penalty)
#   - focus_target_gain=5 (teammate's setting, not 50)
#   - debias_mode=none (val/test patterns too different)
#   - lr=0.0003, dropout=0.1 (teammate settings)
#   - bias_penalty=0.2 (lighter, focus scope)
#   - use_graph=1 (differentiate per-node dynamics)
# ====================================================================
cd "$(dirname "$0")"
